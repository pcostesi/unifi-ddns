...
function getSaneIp(ip, originalIp) {
  // check for ipv6
  if (ip.includes(":")) {
    return ip;
  }
  // check for local ranges. If the IP is local, use the original IP
  if (
    ip.startsWith("10.") ||
...
async function handleRequest(request) {
  ...
  const response = await informAPI(url, username, password, originalIp);
  return response;
}
...
async function informAPI(url, name, token, originalIp) {
  const hostnames = url.searchParams.get("hostname").split(",");
  const rawIp = url.searchParams.get("ip") || url.searchParams.get("myip");
  const saneIp = getSaneIp(rawIp, originalIp);
  ...
  for (const hostname of hostnames) {
    ...
    await cloudflare.updateRecord(record, saneIp);
  }
